...
import { getCategories, getProducts, getTags } from "@/DAL/products";
import { debounce } from "lodash";
import { CategoryCombobox } from "./CategoryCombobox";
import HierarchicalTagSelector from "./HierarchicalTagSelector";
import { createComponentItem, updateComponentItem } from "@/DAL/design";
    ...
    if (isProductGrid) {
      const payload = {
        vals: {
          ...
          product_ids: [[6, 0, selection?.products]],
        },
      };
      try {
        const response = await createComponentItem({ payload });
        const result = response?.data?.result;
        console.log("result for create product grid item", result);
        ...
        console.error(
          "Something went wrong while adding products to the grid:",
          error
        );
      } finally {
        setOpen(false);
      ...
      try {
        ...
        console.log(
          "result for updating selected products to item navigation",
          result
        );
        if (result?.code === 200 && result?.result) {
          const updatedComponent = normalizeComponent(result.result);
        ...
        console.error(
          "Something went wrong while updating selected products:",
          error
        );
      } finally {
        setOpen(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="p-1 w-full bg-blue-500 text-neutral-50 hover:bg-neutral-50 hover:text-blue-500 hover:border-blue-500 border-2 border-transparent rounded transition-colors">
          Select Products
        </Button>
      </DialogTrigger>
      <DialogContent className="min-w-screen h-[90vh] overflow-y-auto p-0 rounded-lg">
        <div className="p-4 bg-black text-white">
          ...
          {!loading && (
            <div className="bg-white text-black ml-4">
              <div className="max-h-[65vh] overflow-y-auto border rounded">
                <Table className="sticky top-0 bg-gray-100 border-t border-b border-gray-600 h-16 z-10">
